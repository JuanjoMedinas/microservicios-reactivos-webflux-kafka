    ...
    public MapReactiveUserDetailsService users() throws Exception {
        List<UserDetails> users = List.of(
                User.withUsername("user1")
                        .password("{noop}user1")//noop para decirle que no está encriptada
                        .roles("USERS")
                        .build(),
                User.withUsername("admin")
                        .password("{noop}admin")//noop para decirle que no está encriptada
                        .roles("USERS", "ADMIN")
                        .build(),
                User.withUsername("user2")
                        .password("{noop}user2")//noop para decirle que no está encriptada
                        .roles("OPERATOR")
                        .build()
        );
